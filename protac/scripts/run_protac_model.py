...
# see .claude/rules/protac-model-pipeline.md ("Arquitectura de invocación") for why these
...
def runFilter(args):
    ...
    # Fix #7: see "Robustness shims" above - poses that fail are now skipped and reported
...
def runRefine(args):
    ...
    # Fix #7, same as runFilter - ros.rosetta() runs the same pose-level Pool.
    try:
